<x-layouts.master-layout>
    <x-show-outer class="flex">
        <div class="w-1/4">
            <x-img src="{{ $actor['profile_path'] }}" alt="" class="w-76" />
        ...
        <div class="ml-8 w-3/4">
            ...
            <div class="grid grid-cols-1 sm:grid:cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
                @foreach ($knownForMovies as $movie)
                    <div class="mt-4">
                        <a href="{{ $movie['link'] }}">
                            <x-img src="{{ $movie['poster_path'] }}" />
                        </a>
                        <a href="{{ $movie['link'] }}" class="text-sm leading-normal bloack text-gray-400 hover:text-white mt-1">
                            {{ $movie['title'] }}
                        </a>
                    </div>
                @endforeach
    ...
    <x-show-outer>
        ...
        <ul class="list-desc leading-loose pl-5 mt-8 border-sm border-l-2">

            @foreach ($credits as $credit)
                <li>{{ $credit['release_year'] }} <strong> &middot; {{ $credit['title'] }}</strong> as {{ $credit['character'] }}</li>
            @endforeach
        </ul>
    </x-show-outer>
